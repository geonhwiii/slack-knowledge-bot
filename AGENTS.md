...
**Read [CONTEXT.md](CONTEXT.md) before writing code.** It holds the domain glossary and the
design decisions this project has already settled — what a Knowledge Entry is, why search is
precision-first, why domain logic stays outside the Slack and framework layers. Use its
vocabulary in code and in conversation, and update it when a term or decision changes.
